        ...
        <form action="{{route('role.update',$role->id)}}" method="POST" id="roleForm" name="roleForm" >
          <input type="hidden" id="id" name="id" value="{{$role->id ?? ''}}">
            ...
            <div class="modal-header">
              ...
              <h4 class="modal-title" id="myModalLabel2">{{__("t.edit_role")}}</h4>
            </div>
          ...
          <div class="col-md-12 col-sm-12 col-xs-12 form-group">
             <label for="case_subtype">{{__("t.role_name")}} <span class="text-danger">*</span></label>
              <input type="text" placeholder="" class="form-control" id="slug" name="slug" value="{{ $role->slug ?? '' }}">
          </div>
            <div class="col-md-12 col-sm-12 col-xs-12 form-group">
              <label for="case_subtype">{{__("t.role_description")}}</label>
